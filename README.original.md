...
If you are currently using ng-completer and interested to maintain it please let me know!

## Installation

```sh
npm install ng-completer --save
```

## Usage

The module you want to use ng-completer in must import `NgCompleterModule` and `FormsModule` (to use the ngModel
directive on ng-completer).  `NgCompleterModule` provides the `CompleterService`, and declares the `ng-completer`
directive.
```ts
...
import { NgCompleterModule } from "ng-completer";
...
Add ng-completer to your component and create a data source:

```ts
import { Component } from '@angular/core';
import { CompleterService, CompleterData } from 'ng-completer';

@Component({
  selector: 'my-component',
  template: `<h1>Search color</h1>
            <ng-completer [(ngModel)]="searchStr" [datasource]="dataService" [minSearchLength]="0"></ng-completer>
            <h1>Search captain</h1>
            <ng-completer [(ngModel)]="captain" [datasource]="captains" [minSearchLength]="0"></ng-completer>`
})
...
ng-completer uses [rxjs](https://github.com/Reactive-Extensions/RxJS) stream as data sources.
...
```ts
   var map = {
       ...
       'ng-completer': 'node_modules/ng-completer/ng-completer.umd.js'
   }
```
